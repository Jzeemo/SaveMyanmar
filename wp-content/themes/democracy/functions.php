...
function wp_bootstrap_starter_widgets_init() {
    ...
    register_sidebar( array(
        'name'          => esc_html__( 'Left Sidebar', 'wp-bootstrap-starter' ),
        'id'            => 'left-sidebar',
        'description'   => esc_html__( 'Add widgets here.', 'wp-bootstrap-starter' ),
        'before_widget' => '<section id="%1$s" class="widget %2$s">',
        'after_widget'  => '</section>',
    ...
    register_sidebar( array(
        'name'          => esc_html__( 'Right Sidebar', 'wp-bootstrap-starter' ),
        'id'            => 'right-sidebar',
        'description'   => esc_html__( 'Add widgets here.', 'wp-bootstrap-starter' ),
        'before_widget' => '<section id="%1$s" class="widget %2$s">',
        'after_widget'  => '</section>',
...
function my_acf_form_init() {
    ...
    if (isset($_POST["acf"])) {
        ...
        $email_address = $data["acf"]["field_601e49cf3908c"];
        ...
        $result = $wpdb->get_results ( "
            ...
            FROM  $wpdb->postmeta
            WHERE meta_key = 'email_address' and meta_value = '".$email_address."'
            " );

        foreach ($result as $count_result) {
            $count = intval($count_result->count);
        }

        if ($count > 0) {

            echo "<script>
            alert('ယခု Email Address ဖြင့် Form submit လုပ်ထားဘူးပါသည်။ နောက်ထပ်တစ်ကြိမ် Submit လုပ်ခွင့်မရှိတော့ပါ။');
            window.location.href='submit-your-voice';
            </script>";
            return false;
